...
- 单实例：应用已在运行时再次启动，第二个实例不创建窗口即退出，并将已有实例的主窗口置前，同时弹出提示。
...
单实例锁以 `tauri.conf.json` 中的 `identifier` 作为互斥体名，开发版与安装版共用同一个标识，因此二者会互相排斥，调试前需先退出已安装的实例。

关闭主窗口时会一并关闭设置窗口。设置窗口是独立的顶层窗口，若其仍然存在，进程不会退出，单实例锁也将继续被占用。
